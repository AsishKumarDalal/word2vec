"""
Full Negative-Sampling (skip-gram) training script
===================================================
Same pipeline as the other scripts (BPE tokenizer, text8/sample corpus, tqdm,
similar words, analogies, plots) but trained with NEGATIVE SAMPLING instead of
full-softmax CrossEntropyLoss.

    per (center, context) pair:
        pos_score  = emb1(center) . emb2(context)
        neg_scores = emb1(center) . emb2(neg1..negK)   (K sampled per unigram^0.75)
        loss = -logsigma(pos) - mean(logsigma(-neg))

Benefits: no full-softmax over the vocab (much faster + less memory), and
better vectors for rare/frequent words.
...
# Reuse analysis/plot helpers from the CBOW script (model-agnostic)
from train import (load_corpus, normalize_rows, similar_words,
                   print_similar_words, show_analogy, print_vectors,
                   pick_query_words, plot_loss_curve, plot_embeddings_2d,
                   plot_embedding_heatmap, plot_similarity_heatmap,
                   plot_similarity_bars, save_embeddings)
...
from Negative_Sampling_Data import NegativeSamplingDataset, NegativeSampler
from Negative_Sampling_model import Negative_Model
...
CONFIG = {
    ...
    "context_size": 2,          # words on each side of the center word
    "max_length": 256,
    "vocab_size": 10000,        # target BPE vocab size
    "num_negatives": 5,         # K negative samples per pair (typical: 5-20)
    "batch_size": 256,
    "epochs": 15,
    "lr": 0.002,                # NS converges well with a slightly higher LR
    "seed": 42,
...
def get_embeddings(model):
    """Final word vectors = average of input (emb1) + output (emb2)."""
    E1 = model.emb1.weight.detach().cpu().numpy()   # (V, E)
    E2 = model.emb2.weight.detach().cpu().numpy()   # (V, E)
    return (E1 + E2) / 2.0
...
def main():
    ...
    print("=" * 60)
    print(f"SKIP-GRAM + NEGATIVE SAMPLING training | device: {device}")
    print("=" * 60)
    ...
    # 3) Dataset + negative sampler
    dataset = NegativeSamplingDataset(texts, tokenizer,
                                      context_size=cfg["context_size"],
                                      max_length=cfg["max_length"],
                                      max_pairs=cfg["max_pairs"],
                                      seed=cfg["seed"])
    dataloader = DataLoader(dataset, batch_size=cfg["batch_size"], shuffle=True)
    sampler = NegativeSampler(tokenizer, texts)
    print(f"Training (center, context) pairs: {len(dataset)}")

    # 4) Model
    model = Negative_Model(vocab=tokenizer.vocab_size, emb=cfg["emb_size"]).to(device)
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        for center, context in batch_bar:
            negatives = sampler.sample(center.shape[0], cfg["num_negatives"],
                                       exclude=context)
            ...
            optimizer.zero_grad()
            loss = model.ns_loss(center, context, negatives)
            loss.backward()
            optimizer.step()
    ...
    torch.save({
        ...
        "config": cfg,
    }, "negative_sampling_model.pt")
    print("\n" + "=" * 60)
    print(f"TRAINING COMPLETE | final loss: {losses[-1]:.4f}")
    print("Model saved -> negative_sampling_model.pt")
    print("=" * 60)
    ...
    print("  negative_sampling_model.pt")
    print("=" * 60)
